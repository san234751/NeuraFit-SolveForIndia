...
class _RenderDataPlankState extends State<RenderDataPlank> {
  ...
  bool _postureAccordingToExercise(Map<String, List<double>> poses) {
    setState(() {
      rightAnklePosX = poses['rightAnkle']![0];
      rightAnklePosY = poses['rightAnkle']![1];

      rightkneePosX = poses['rightKnee']![0];
      rightkneePosY = poses['rightKnee']![1];

      righthipPosX = poses['rightHip']![0];
      righthipPosY = poses['rightHip']![1];

      rightshoulderPosX = poses['rightShoulder']![0];
      rightshoulderPosY = poses['rightShoulder']![1];
    ...
    if (excercise == 'plank') {
      double slope1 = (rightAnklePosY! - rightkneePosY!) /
          (rightAnklePosX! - rightkneePosX!);
      double slope2 = (righthipPosY! - rightshoulderPosY!) /
          (righthipPosX! - rightshoulderPosX!);
      double slope3 =
          (rightkneePosY! - righthipPosY!) / (rightkneePosX! - righthipPosX!);
      print(
          '//////////////////////////// the color is fuck you ${(slope1 - slope2).abs() / slope1}');

      // if ((slope1 - slope2).abs() / slope1.abs() < 0.1 &&
      //     (slope2 - slope3).abs() / slope2.abs() < 0.1 &&
      //     (slope1 - slope3).abs() / slope1.abs() < 0.1) {
      //   return true;
      // }

      //angle between slope1 and slope2
      var angle =
          math.atan((slope1 - slope2) / (1 + slope1 * slope2)) * 180 / math.pi;
